...
props.Colonias.map(function(value, key) {
...
const eliminarRefugio = () => {
    form.post(route('colonia.destroy',form.id),{
        onSuccess: () => {
...
function itemSeleccionado() {
    ...
    dt.rows({ selected: true }).every(function () {
        ...
        if (idx >= 0) {
            ...
            console.log(form.id + ' ' + data.value[idx].id,' => ',data.value[idx].colonia);
        }
    });
}
...
<!--        </div>-->
		...
		<div class="p-4 bg-white rounded-lg shadow-xs">
			<div class="inline-flex overflow-hidden mb-4 w-full bg-white rounded-lg shadow-md">
                <NavLink :href="route('colonia.create')" :titulo="'Nueva colonia'">
...
<!--                <PrimaryButton :show="showButtonXLSX" @click="exportXSLX($event)" title="Editar colonia" class="ml-1 mt-1" :type="'default'" :classBtn="'px-3 py-2 mt-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-emerald-600 border border-transparent rounded-lg active:bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:shadow-outline-emerald'">-->
<!--                    <slot name="icon">-->
<!--                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">-->
...
<!--                    </slot>-->
<!--                </PrimaryButton>-->
<!--                <PrimaryButton :show="showButtonPoint" @click="openModalEliminar(data)" class="mb-2" title="Eliminar colonia" :class-btn="'px-3 py-2 ml-3 mt-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-lg active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red'">-->
...
<!--                </PrimaryButton>-->
                    ...
                    >
						<thead>
							<tr class="text-xs font-bold tracking-wide text-left text-blue-500 uppercase bg-gray-300 border-b">
                                ...
                                <th class="px-4 py-3">COMUNIDAD</th>
							</tr>
						</thead>
					</DataTable>
				</div>
        ...
        <Modal :show="showModalEliminar" @close="closeModalEliminar">
            <div class="p-6">
                <p>
                    Quiéres eliminar este colonia
                    <span class="text-2x1 font-medium text-gray-900">{{data.colonia}} ? </span>
                </p>
